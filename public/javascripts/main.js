...
function startweb(){
    ...
    // khi màn hình scroll thì xử lý ẩn - hiện một số thành phần trên header (đã sử dụng)
    ...
    if($(".post-wrapper").length){ //đối với những trang có phần bài post thì:
        // thiết lập nhận comment thì có comment mới từ server qua socket
        setGiveComment();

        // thiết lập nhận thông báo có bài viết bị xóa từ server qua socket
        setGiveDeletePost();
    }
    ...
    //khi nhấn bất cứ thành phần giao diện nào có class show-my-loading thì hiển thị modal loading lên
    $(".show-my-loading").bind("click",function(){
...
function addNewVideoIndex(data){
    var idmodal = data.id;
    //thêm một input mới vào cái khu vực thêm liên kết video
    ...
    // console.log($(`#${idmodal} .input-video-group .add-new-index-video`))
    $(`#${idmodal} .input-video-group`).append(newVideoInput)
}
...
/*start: phần đăng nhập (không sử dụng nữa - thay thế = passport trong backedn) */
 ...
 function onSuccess(googleUser) { // khi đăng nhập thành công -- chỉ khi ta thay đổi email hay đăng nhập một cái mới
...
function renderButton() {
    gapi.signin2.render('my-signin2', {
        ...
        'longtitle': true,
        'theme': 'dark',
        'onsuccess': onSuccess, /* chỉ định tên hàm khi đăng nhập thành công */
        'onfailure': onFailure /* chỉ định tên hàm khi đăng nhập thất bại */
    });
}
  
  /* định dạng cho cái google button */

function onSignIn(googleUser) { /* khi đang đăng nhập -- đăng nhập lần đầu hàm này sẽ không chạy mà hàm onSuccess sẽ chạy */
    ...
    // var checkmail = checkStudentEmail(profile.getEmail());
  ...
  function verifyGoogleEmail(id_token){ /* gửi id_token của google email để backend kiểm tra trước khi đăng nhập */
    $.post("/verify",{id_token:id_token})
    .then(result=>{ // có result => code >=200 và <300 nghĩa là cái id_token (dùng để kiểm tra xác thực và kiểm email) ổn
    ...
    .catch(err=>{ // vào đây => code != 20x => có lỗi
...
function showAlert(idAlert, content, action){ //action (k bắt buộc) là một hàm thực hiện chức năng khác -- như ẩn đi alert
    if(!$(`#${idAlert}`).hasClass("show")){
        $(`#${idAlert}`).addClass("show")
    }
    $(`#${idAlert}`).empty();
    $(`#${idAlert}`).text(content)
    if(action){ // nếu có một hàm hành động thì thực hiện nó
        action
    }
}
function hideAlert(idAlert,content,action){
    $(`#${idAlert}`).html(null);
    $(`#${idAlert}`).removeClass("show");
}
...
function showImageBefor(input) { // khi input file hình được up lên html thì hiện lên lên cho người dùng (đã sử dụng)
...
//(đã sử dụng)
async function showImagesBefor(input, data){ // khi input file hình được up lên html thì hiện lên cho nguiowf dùng
    ...
    var modalid = data.id; //id của cái modal (thêm hoặc sửa)
    ...
    if(input.files && input.files.length){
        ...
        for(var i =0 ; i<n;i++){
            ...
            // gọi promise đọc file và await chờ nó
            await myreadfiles(file)
            .then((result)=>{
                var e =result
                var fileidtmp = Date.now(); // tạo ra một id theo thiowf gian để không bị trùng
                var content = `<div class="file-show-input file-add-before-upload" id="file-add-upload-${fileidtmp}">
                                    <img src="${e.target.result}" class="d-block w-100">
                                    <div onclick="deleteImagesFromPostInput(this);" class="delete-file-show-input" data-id="file-add-upload-${fileidtmp}" data-modalid="${modalid}"><i class="far fa-times-circle"></i></div>
                                    <hr>
                                </div>`
                $(`#${modalid} .img-input-after`).append(content)
                // thêm vào một giá trị json để dễ dàng có thể tương tác thông qua id
                var fileinfor = {
                    id: `file-add-upload-${fileidtmp}`,
                    file: file
                }
                 //vì có 2 dạng thêm và sửa nên mảng input cũng sẽ khác nhau -- do đó tùy thuộc thêm hay sửa mà làm cho phù hợp
                if(modalid=="editpostModal"){ //nếu là sửa
...
const myreadfiles = function(file){
...
reader_information_image.onload = function (e) { // khi load xong thì hiện lên (đã sử dụng)
...
function showConfirmDeleteNotificationFile(file){//file = this
    //ẩn loading
    hideLoading("delete-file-notification-loading");
    var data = file.dataset;
    // lấy id bài thông báo được gắn ở form
    var notificationid = $(file).parent().parent().parent().parent().parent().attr("data-id");
    
    // lấy thông tin tên file và id file để hiển thị lên view cho người dùng xem
    var {fileid, originfilename} = data;
    $("#confirm-delete-notification-file .modal-body").html(originfilename)
    // gắn id file
    $("#confirm-delete-notification-file .confirmed-delete-notification-file").attr("data-fileid",fileid);
    // gắn id bài thông báo
    $("#confirm-delete-notification-file .confirmed-delete-notification-file").attr("data-notificationid",notificationid);
    //hiện modal lên
...
function confirmedDeleteNotificationFile(data){
    //hiện loading
    showLoading("delete-file-notification-loading");
    // lấy ra id của file
    var id = data.fileid;
    // lấy ra id của bài thông báo
    var notificationid = data.notificationid;
    // sử dụng href để xóa (fileid - là id của file, còn id là id của bài thông báo)
    window.location.href = `/notification/deletefile?fileid=${id}&id=${notificationid}`;
}

//hiển thị modal xác nhận xóa bài thông báo (đã sử dụng)
function showConfirmDeleteNotification(notification){ // notification = this
    //ẩn loading
    hideLoading("delete-notification-loading");
    var data = notification.dataset;
    //lấy ra id thông báo
    var id = data.id;
    $("#confirm-delete-notification").modal("show");
    // thêm nội dung
    $("#confirm-delete-notification .modal-body").html("Bạn có chắc muốn xóa bài thông báo này không ? Việc xóa này có thể xóa đi tất cả nội dung và file liên quan.")
    // gán id thông báo để xóa
...
function confirmedDeleteNotification(notification){
    //hiện loading
    showLoading("delete-notification-loading");
    console.log(notification)
    
    var data = notification.dataset
    //lấy ra id bài thông báo
    var id = data.id
    // tiến hành gọi href để xóa
...
function setGiveNotification(){
    ...
    socket.on(`server-send-new-notification`,function(data){
        // console.log(data);
        // data được gửi là một thông báo
        var notification = data;
        var returnValue=``;
        if(window.location.pathname==="/home"){ // nếu người dùng đang ở trang home thì thêm thông báo mới vào cái ô các thông báo nằm bên phải
            if(notification.department.departtype=="K"){
                ...
                <div class="notification notification-odd">
                    ...
                    <div class="sm-text notification-title"> <!--Phần tiêu đề-->
                        ${notification.title} <span class="badge my-badege-1">Mới</span>
                    </div>
                </div>
                </a>`
            }
        ...
        message += `${notification.department.departcode} vừa đăng một thông báo mới: 
        <a class="blue-text" href="/notification/details?id=${notification.id}">${notification.title}</a>`
...
function showModalMiniPost(post){ //post = this
   ...
   var top_post =($(`#${id} .top-post`).html());
   
   $("#show-options-imgs-videos-post .top-post").html(top_post)
    //xóa đi cái khung chỉnh sửa - xóa
    $("#show-options-imgs-videos-post .dropdown").remove();
   //lấy phần nội dung
   var content_post_wrapper = $(`#${id} .content-post-wrapper`).html()

   $("#show-options-imgs-videos-post .content-post-wrapper").html(content_post_wrapper)

   //lấy phần hỉnh ảnh và video để hiển thị dạng slide
   var options_img_videos_wrapper = $(`#${id} .options-img-videos-wrapper div`)
//    options_img_videos_wrapper.forEach(function(child){
//        console.log(child)
//    })
...
function deleteImagesFromPostInput(image){
    ...
    var data = image.dataset;
                                                // và id của cái modal (modal của thêm post hoặc sửa post)
    ...
    //vì có 2 dạng thêm và sửa nên mảng input cũng sẽ khác nhau -- do đó tùy thuộc thêm hay sửa mà làm cho phù hợp
    if(modalid=="editpostModal"){ //nếu là modal sửa
        //tìm vị trí trước
        var indexToDelete = filesToUploadEdit.findIndex(t=>t.id==id);
        // nếu có mới xóa
        if(indexToDelete >=0){
    ...
    else{ // nếu là modal thêm
        //tìm vị trí trước
        var indexToDelete = filesToUpload.findIndex(t=>t.id==id);
        // nếu có mới xóa
        if(indexToDelete >=0){
...
function deleteImagesFromPostInputAfter(image){
    ...
    console.log(idImagesToDelete)
    //xóa khỏi view (khác với xóa hình khi mới upload khi thêm mới)
    $(image).parent().remove();
}
...
function deleteIndexVideoAfter(videoindex){
    ...
    idVideosToDelete.push(videoid)
    //xóa đó gọi hàm xóa đi cái ô input
...
function addPost(){
    //hiện loading
    showLoading("add-post-loading");
    //chuẩn bị một formdata
    var formData = new FormData();
    //nếu có file hình để gửi thì thêm vào form (mảng hình lấy từ filesToUpload)
    ...
    // tiếp theo là duyệt các ô input videos xem ô nào có value thì lấy ra và append vào form data
    ...
    $.ajax({
        ...
        url:"/post/add",
        data:formData,
        processData: false, // nếu muốn post form data lên backend = ajax thì nhớ thêm 2 thông số này
        contentType: false,
    })
    .then(result=>{
        ...
        addPostView(post)
        // clear (làm sạch) cái modal thêm
        clearPostModal("postModal");
        // clear cái mảng input files global
        filesToUpload = []
        //ẩn loading
        hideLoading("add-post-loading");
        // hạ modal thêm xuống
...
function clearPostModal(postmodalid,option){
    ...
    $(`#${postmodalid} input`).val(null);
    $(`#${postmodalid} textarea`).val(null);
    // làm mới lại mấy ô thêm input videos
    if(option==2){ // nếu == 2( nghĩa là làm mới form chỉnh sửa bài post -- ta không cần thêm cái input video)
        $(`#${postmodalid} .input-video-group`).html(`<label class="blue-text">Liên kết video</label>`)
    }
    else{//ngược lại thì thêm
        $(`#${postmodalid} .input-video-group`).html(`<label class="blue-text">Liên kết video</label>
    ...
    $(`#${postmodalid} .img-input-after`).html(null);
    // ẩn luôn cái alert
...
function addPostView(post,option){ // result.data = post
    ...
    <div class="comment-post"> <!--Chỗ nhập bình luận của bản thân-->
      <a href="/wall?id=${owner.id}" class="me-2">
          <img src="${information.avatar}" class="avatar-img-round avatar-img-round-comment">
      </a>
    ...
    if(option==2){ // gán lên view dành cho lúc load timeline (gán vào cuối view)
        

        $(".post-wrapper").append(toview)
    }
    else{ // gán lên view dành cho lúc tạo mới post (gán vào đầu view)
...
function showModalEditPost(post, event){ // post = this
    //ẩn đi cái loading
    hideLoading("edit-post-loading");
    event.preventDefault();
    //ẩn đi cái modal hiển thị bài post dạng khác
    $("#show-options-imgs-videos-post").modal("hide")
    //ẩn cái alert
    ...
    // gắn id của bài viết vào nút xác nhận sửa để tiện cho việc lấy thông tin
    $("#editpostModal #editpostconfirm").attr("data-postid",postid);

    //lấy dữ liệu từ post được thêm
    //lấy content
    var content = $(`#${postid} .content-post`).html();
    //lấy ra hình ảnh và video
    var images = $(`#${postid} .options-img-videos-wrapper .options-post-child img`);
    var videos = $(`#${postid} .options-img-videos-wrapper .video-post-wrapper iframe`);
    
    //thêm dữ liệu cho modal chỉnh sửa
    //thêm content bài post
    $("#editpostModal textarea").val(content)
    //thêm các liên kết videos vào ô input (nếu có)
    if(videos.length){
        var n = videos.length;
        for(var i = 0; i<n ; i++){
            var video = videos[i];
            ...
            var newVideoInput = `<div class="d-flex flex-row">
                                    <input disabled class="my-input me-2" type="text" placeholder="Liên kết video" value="${videolink}">
                                    <div data-id="${videoid}" onclick="deleteIndexVideoAfter(this);" class="delete-index-video-show mt-2"><i class="fas fa-trash"></i></div>
                                </div>`
    ...
    if(images.length){
        ...
        for(var i = 0; i<n ; i++){
            ...
            //thêm vào khu vực hiển thị
            var newImageInput = `<div class="file-show-input file-add-before-upload" id="${imageid}">
                                    <img src="${imagelink}" class="d-block w-100">
                                    <div onclick="deleteImagesFromPostInputAfter(this);" class="delete-file-show-input" data-id="${imageid}" data-modalid="editpostModal"><i class="far fa-times-circle"></i></div>
                                    <hr>
                                </div>`
...
function showModalDeleteComment(comment,event){// comment = this
    event.preventDefault();
...
function editPost(post){
    ...
    //chuẩn bị một formdata
    var formData = new FormData();
    //nếu có file hình để gửi thì thêm vào form (mảng hình lấy từ filesToUploadEdit)
    ...
    formData.set("content",content);
    console.log(content)
    // tiếp theo là duyệt các ô input videos xem, nếu người dùng có thêm videos mới thì lấy ra
...
function editPostView(post){
    ...
    attachedfiles.forEach(function(file){
        if(file.filetype=="image"){
            filesToShow+=`<div class="options-post-child" data-id="${file.id}" onclick="showModalMiniPost(this);">
                            <img id="${file.id}" src="${file.fileurl}" class="d-block w-100">
                        </div>
                        <hr>`
        }
        else{
            filesToShow+=` <div class="video-post-wrapper" data-id="${file.id}">
                                <iframe data-url="${file.fileurl}" id="${file.id}" width="100%" height="350" src="https://www.youtube.com/embed/${convertYoutubeUrl(file.fileurl)}" allowfullscreen></iframe>
                            </div>
                            <hr>`
        }
    ...
    $(`#${postid} .content-post`).html(content); // cập nhật content mới
    $(`#${postid} .options-img-videos-wrapper`).html(filesToShow); // cập nhật hình ảnh và video mới
}

//hàm hiển thị modal xác nhận xóa bài post khi người dùng nhấn xóa một bài post (onclick trong html) (đã sử dụng)
function showModalDeletePost(post,event){// post = this
    //ẩn loading
    hideLoading("delete-post-loading");
    event.preventDefault();
    var postid = post.dataset.id;
    
    //gắn id bài viết vào nút confirmed để xóa dễ dàng
...
function confirmedDeletPost(post){ //post = this
    //hiện loading
    showLoading("delete-post-loading");
    //lấy ra id bài post
    ...
    .then(result=>{
        ...
        deletePost(id);
        //ẩn đi cái modal
        $("#confirm-delete-post-modal").modal("hide");
        //ẩn loading
        hideLoading("delete-post-loading");
        //hiển thị một htonog báo thành công
        showToast("post-toast","Xóa bài viết thành công")
    })
    .catch(err=>{ // nếu có lỗi thì show lên toast
        err = JSON.stringify(err);
        err = JSON.parse(err)
        var mess = err.responseJSON.message;
        //ẩn loading
...
function typeComment(inputtype,event){ //inputtype = this

    var keycode = (event.keyCode ? event.keyCode : event.which); // lấy keycode của nút được nhấn
    if(keycode == '13'){  // 13 là nút Enter -- Enter mới thực hiện việc lấy value ra
        //lấy ra id của bài viết được comment
        var postid = inputtype.dataset.id;
        // lấy ra nội dung comment sau khi nhấn nút enter
        var commentContent = $(inputtype).val();
        $(inputtype).val(null);
        // nếu nội dung comment không bị rỗng mới thực hiện up lên server
...
function postComment(postid, commentContent){
    ...
    .then(result=>{
        //nếu thành công thì cập nhật lên view người dùng
...
function postCommentView(postid, comment){
    ...
    $(`#${postid} .comment-post-wrapper .not-comment-alert`).remove();
    // console.log(comment)
    ...
    var editadd =``;
    //nếu người dùng hiện tại là chủ nhân comment
    if(rolecode=="AM" || userid == owner.id){
        editadd += `<div class="comment-edit"> <!--CHỉ hiển thị đối với comment của bản thân hoặc admin-->
                    <!--Phần dropdown dùng để hiển thị lựa chọn chỉnh sửa hoặc xóa -- chỉ dành cho bài viết của bản thân-->
                    <div class="dropdown">
                    <button class="btn btn-secondary sm-text dropdown-post" type="button" id="dropdownPost" data-bs-toggle="dropdown" aria-expanded="false">
                    ...
                    <ul class="dropdown-menu" aria-labelledby="dropdownPost">
                        <li><a class="dropdown-item deletecomment" href="#" data-id="${comment.id}" onclick="confirmDeleteComment(this,event)">Xóa</a></li>
                    </ul>
                    </div>
                </div>`
    }
    
    var contentToAdd = `<div class="comment-post" id="${comment.id}">
    <a href="/wall?id=${owner.id}" class="avatar-post-wrapper me-2">
        <img src="${information.avatar}" class="avatar-img-round avatar-img-round-comment">
    </a>
    <div class="comment me-2">
      <div class="content-comment"> <!--nội dung comment gồm tên người cmt và nội dung cmt-->
        <div class="bold-text"><a href="/wall?id=${owner.id}">${information.showname}</a></div>
    ...
    $(`#${postid} .comment-post-wrapper`).prepend(contentToAdd);
}

//hàm hiển thị modal xác nhận xóa comment (đã sử dụng)
function confirmDeleteComment(comment, event){ // this=comment
    //ẩn loading
    hideLoading("delete-comment-loading");
    event.preventDefault();
    //lấy ra id của comment
    var commentid = comment.dataset.id;
    //gắn id vào nút xác nhận xóa của modal xác nhận xóa comment
    $("#confirm-delete-comment-modal #confirmed-delete-comment-modal").attr("data-id",commentid);
    //hiển thị modal xác nhận xóa
    $("#confirm-delete-comment-modal").modal("show");
}
//khi nhấn nút xác nhận xóa một comment (đã sử dụng)
function confirmedDeleteComment(comment){ // comment = this
    //hiện loading
    showLoading("delete-comment-loading");
    //lấy ra id của comment
    var commentid = comment.dataset.id;
    //lấy ra id của bài post
    var postid = $(`#${commentid}`).parent().parent().attr("id");
    //thực hiện gọi ajax xóa comment
    $.ajax({
        url:`/post/deletecomment/${postid}`,
        type:"post",
        data: {
            commentid:commentid
        }
    ...
    .then(result=>{
        ...
        deleteCommentView(result.data);
        //ẩn loading
        hideLoading("delete-comment-loading");
        
        //ẩn đi cái modal xác nhận
    ...
    .catch(err=>{
        ...
        err = JSON.parse(err)
        var mess = err.responseJSON.message;
        //gọi hàm hiển thị modal lưu ý lỗi và ẩn đi cái modal xác nhận
...
function setGiveComment(){
    //thiết lập nhận thông báo realtime khi có một thông báo mới được thêm vào
    var userid = $("#user").attr("data-id");
    socket.on(`server-send-new-comment`,function(data){
        var comment = data;
...
function getMoreComment(commentclick,event){ //commentclick = this
    //event = event
    event.preventDefault();
    // console.log(commentclick.dataset)
    var postid = commentclick.dataset.id;
    //tạm thời ẩn đi cái chữ "Xem thêm bình luận" để người dùng không nhấn lần 2
    $(`#${postid} .action-comment`).html(null);
    //dùng ajax để lấy tất cả comments của bài viết
    $.ajax({
        url:`/post/comment/${postid}`,
        type:"get"
    })
    .then(result=>{
        ...
        $(`#${postid} .comment-post-wrapper`).html(null);
        var comments = result.data;
        //nếu không có giá trị gì thì gắn vào khu vực comment này một câu lưu ý
        if(!comments.length){
            $(`#${postid} .comment-post-wrapper`).html(`<div class="text-center red-text not-comment-alert"> Không có comment để hiển thị </div>`);
            //hiển thị lại chữ "Xem thêm bình luận"
            $(`#${postid} .action-comment`).html(`<a data-id="${postid}" href="#" class="blue-text more-comment" onclick="getMoreComment(this,event);">Xem thêm bình luận</a>`);
        }
        else{ // nếu có thì hiện lên view
            ...
            $(`#${postid} .action-comment`).html(`<a data-id="${postid}" href="#" class="blue-text more-comment" onclick="hideComments(this,event);">Ẩn bớt bình luận</a>`);
    ...
    .catch(err=>{
        ...
        err = JSON.parse(err)
        var mess = err.responseJSON.message;
        //gọi hàm hiển thị modal lưu ý lỗi
        showLocalModal(mess)
    })
...
async function loadMorePost(){
    ...
    var isWall = location.pathname;
    var userid;
    if(isWall.includes("/wall")){
        var queryval = location.search; // lấy tham số trên url (có bao gồm phần ?..=)
    ...
    var post_wrapper = document.getElementsByClassName("post-wrapper");
    // lấy ra số lượng bài post hiện tại được gắn trên html
    var postsLength = post_wrapper[0].dataset.postslength;
    postsLength = parseInt(postsLength);
    // dùng ajax tiếp tục gọi thêm bài post từ server (nhiều nhất là 10)
    var maxPostsToGet = 10;
    // tạo mảng chứa các bài posts sẽ được up lên
    var postsArray = [];
    var i = 0;
    //hiển thị loading
    showLoading("get-post-loading")
    while(i<maxPostsToGet){
        var skip = postsLength;
        var limit = 1;
        // thực hiện lấy bài post (ta gọi api lấy số nhiều nhưng limit 1 thì mảng sẽ có 1 phân tử)
        var posts = await loadPostBySkipAndLimit(skip,limit,userid)
        // console.log(posts);
        // break;
        //nếu không còn post thì ngưng -- không cần lấy thêm
        if(!posts.length){
            break;
        }
        var post = posts[0]; //mảng posts chỉ có 1 phần tử là post ta cần tìm nên ta lấy nó ra ở vị trí 0
        // console.log($(`#${post.id}`))
        //nếu post này được lấy về mà bị trùng với cái đã có thì bỏ qua nó và tằng giá trị tiếp tục lấy cái khác
        if($(`#${post.id}`).length){ // kiểm tra phần tử có tồn tại chưa bằng cách thêm .length
...
function setGiveDeletePost(){
    ...
    var userid = $("#user").attr("data-id");
    socket.on(`server-send-delete-post}`,function(data){
        //lấy giá trị số lượng bài viết hiện tại (được gắn trên html khi mới khởi động trang)
        // ta sẽ không sài jquery vì nó rất dễ lỗi khi get, set các giá trị dataset
        var post_wrapper = document.getElementsByClassName("post-wrapper");
        // lấy ra số lượng bài post hiện tại được gắn trên html
        var postsLength = post_wrapper[0].dataset.postslength;
        //giảm giá trị postsLength để lát skip cho phần timeline chính xác hơn
...
//hàm hiển thị một loading
function showLoading(loadingid){
    $(`#${loadingid}`).removeClass("my-hide");
}
//hàm ẩn một loading
function hideLoading(loadingid){
    if(!$(`#${loadingid}`).hasClass("my-hide")){
        $(`#${loadingid}`).addClass("my-hide")
    }
}